...
- The **description** is sealed with the same key, in a column of its own. A
  group has no public side at all — no handle, no discovery, no invite page a
  stranger reads — so there is no reason its text should be readable here and
  every reason it should not: a sentence describing what a group is for says as
  much about the people in it as the name does.
...
**The picture is the exception, and it is deliberate.** A group's picture is
stored unsealed and withheld instead: the server hands it to the members of the
group that points at it and to nobody else, not even to somebody holding the
media id. That is a weaker promise than encryption and it is the one being
made. The reason is the same one migration 023 gives for a channel's: a picture
is a label on a door rather than a message. It has to be drawable the moment
the group is on screen — including on a device that joined by a link and has
not been sent the group key yet — and it must not be lost to a key rotation
that raced the upload. The messages stay end-to-end encrypted; the picture on
the door does not. Migration 036 is where this is written down.

Only admins may change the name, the description or the picture, checked on the
device and again at the server.

The limits, stated: a member who is removed keeps the group key, so they could
still open a name and a description they already had — neither is re-keyed on
removal — and a picture they had already fetched is a file on their phone.
